...
// /home/alex/buildroot/output/host/bin/arm-buildroot-linux-uclibcgnueabihf-gcc muxparam.c -o muxparam
// This program assumes both RW and RO block uses the same size (64KB).
#define BLOCK_SIZE 0x10000 //64KB

#define FLASH_ENV_NAME "/dev/mtdblock1"
...
{	
	...
	if (strncmp(argv[1], "dump", 4) == 0)
	{
		...
		if (argc != 2)
		{
			...
			{
				printf("usage:\n");
				printf("muxparam dump\n");
				ret = -1;
				goto done;
			}
		}		
		
		fdenv = open(FLASH_ENV_NAME, O_RDONLY);
		...
		len = read(fdenv, buffer, BLOCK_SIZE);
		if (len != BLOCK_SIZE)
		{
	...
	{		
		...
		{
			printf("usage:\n");
			printf("muxparam s <param_name> [<param_value>]\n");
			ret = -1;
			goto done;
		}		
		
		fdenv = open(FLASH_ENV_NAME, O_RDWR);
		if (fdenv < 0)
		{
			printf("failed to open %s\n", FLASH_ENV_NAME);
			ret = -2;
		...
		{
			printf("Current CRC is incorrect!!! Exit.\n");			
			ret = -5;
			goto done;
		...
		// Update CRC		
		*(unsigned int*)buffer = crc32(0, buffer + 4, BLOCK_SIZE - 4);		
		lseek(fdenv, 0, SEEK_SET);
		len_to_write = get_len_to_write(buffer);		
		len = write(fdenv, buffer, len_to_write);
		if (len != len_to_write) 
		{
			printf("failed to write %d bytes data to %s\n", len_to_write, FLASH_ENV_NAME);
			ret = -6;
			goto done;
		}				
	}
