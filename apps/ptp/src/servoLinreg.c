...
static void _linregDestroy(struct servo *servo)
{
...
static double _linregSample(struct servo *servo,
			    int64_t offset,
			    uint64_t local_ts,
			    double weight,
...
static void _linregSyncInterval(struct servo *servo, double interval)
{
	struct linreg_servo *s = container_of(servo, struct linreg_servo, servo);
...
static void _linregReset(struct servo *servo)
{
	struct linreg_servo *s = container_of(servo, struct linreg_servo, servo);
	unsigned int i;
...
static double _linregRateRatio(struct servo *servo)
{
	struct linreg_servo *s = container_of(servo, struct linreg_servo, servo);
...
static void _linregLeap(struct servo *servo, int leap)
{
	struct linreg_servo *s = container_of(servo, struct linreg_servo, servo);
...
struct servo *linreg_servo_create(int fadj)
{
	...
	s->servo.destroy = _linregDestroy;
	s->servo.sample = _linregSample;
	s->servo.sync_interval = _linregSyncInterval;
	s->servo.reset = _linregReset;
	s->servo.rate_ratio = _linregRateRatio;
	s->servo.leap = _linregLeap;
