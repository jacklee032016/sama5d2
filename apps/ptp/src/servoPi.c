...
struct pi_servo
{
	...
	/* configuration: */
	/*
	* 2 items configured in file; when these 2 is configured as non-0, others configurations are not used
	* used to fill the configured_pi_kx_scale 
	*/
			...
			enum servo_state *state)
{
	...
	switch (s->count)
	{
		...
		case 1:
			...
			/* diff between local continuous timestamps */
			...
			/* after first into JUMP state, it never enter JUMP state again; normally enter into LOCKED directly after locked first time */
			if ((servo->first_update && servo->first_step_threshold && servo->first_step_threshold < fabs(offset)) ||
		...
		case 2:
			 ...
			 * clock startup.
			 */
			if (servo->step_threshold && servo->step_threshold < fabs(offset))
			{
				*state = SERVO_UNLOCKED;
				s->count = 0;
				break;
...
static void pi_reset(struct servo *servo)
{
	...
	/* add to accelebrate sync process. 12.05, 2019*/
...
struct servo *pi_servo_create(struct PtpConfig *cfg, int fadj, int sw_ts)
{
	...
	s->drift         = fadj;
	s->last_freq     = fadj;/* freq from OS */
	s->kp            = 0.0;
	s->ki            = 0.0;

	/* normal configure following 2 items for Kp and Ki*/
