...
//    UINT32 Elapsed = 0;
//    return (Elapsed);
    *CurrMsCount = (1000 / configTICK_RATE_HZ) * xTaskGetTickCount();
    return (*CurrMsCount - StartCount);
}
...
//    UINT32 i = 0;
//    return(i);
	return ( (1000 / configTICK_RATE_HZ) * xTaskGetTickCount() );
}
...
UCHAR  data;
	...
	data = twi_mux_Read8(ADV7619_chan, DevAddr>>1, RegAddr);
...
UCHAR  data;
	...
	data = twi_mux_Read8(ADV7619_chan, DevAddr>>1, RegAddr);
	data = (data & ~Mask) | ((FieldVal << BitPos) & Mask);

	twi_mux_Write8(ADV7619_chan, DevAddr>>1, RegAddr, data);
}
...
int i, idx;
	...
	tx[0] = RegAddr;
	twi_mux_master_transfer(ADV7619_chan, DevAddr>>1, 1, tx, FldSpan, data );
...
int i, idx;
	...
	data[0] = RegAddr;
	twi_mux_master_transfer(ADV7619_chan, DevAddr>>1, 1, data, FldSpan, data+1 );
	idx = 1;
	for (i=0; i<FldSpan; i++) {
	...
	data[0] = RegAddr;
	twi_mux_master_transfer(ADV7619_chan, DevAddr>>1, 1+FldSpan, data, 0, data );
...
int i = 0;
	...
	while (Table[i]!= EndVal)
	{
		twi_mux_Write8(ADV7619_chan, Table[i]>>1, Table[i+1], Table[i+2]);
		i += 3;
	}
}
...
U8   nb;
	...
	for (i=0; i<NofBytes; i+=16) {
		...
		tx[0] = Reg+idx;
		twi_mux_master_transfer(ADV7619_chan, Dev>>1, 1, tx, nb,  Data+idx );
...
U8   nb;
	...
	for (i=0; i<NumberBytes; i+=16) {
		...
		tx[0] = Reg+idx;
		memcpy(tx+1, Data+idx, nb);
		twi_mux_master_transfer(ADV7619_chan, Dev>>1, 1+nb, tx, 0, tx);
...
{
	*Data = twi_mux_Read8(ADV7619_chan, Dev>>1, Reg );
	return 1;
}
...
{
	twi_mux_Write8(ADV7619_chan, Dev>>1, Reg, Data);
    return 1;
}
